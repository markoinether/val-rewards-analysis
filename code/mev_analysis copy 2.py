...
def print_percentile_stats(start_percentile, end_percentile):
    for percentile in range(start_percentile, end_percentile):
        ...
        print(f"Percentile {percentile}-{percentile + 1}:")
        ...
        print(f"  Mean: {percentile_mean:.2f}")
        print(f"  Median: {percentile_median:.2f}")
        print(f"  Standard Error: {percentile_std_err:.2f}")
...
for i in range(0, 100, 10):
    print_percentile_stats(i, i + 10)

# Print stats for each individual percentile from 91 to 100
print_percentile_stats(91, 100)
